...
926 spoken words. That is 5 minutes 9 seconds at 180 words a minute, which is a
...
> hundred rupees. Wrongly blocking a real customer costs fifteen thousand.
> Seventy five to one.
>
...
> between strangers, so it carries nothing, and the model says so.

**Switch to the API.** `POST /v1/scan`, twelve thousand accounts.

> Raw fields a merchant already has. One and a third seconds. Eighty six percent
> of that batch's ring accounts reached a human, six innocent accounts swept up.
...
**On screen:** `results/adaptive_visibility_replicates.png`, the zoomed panel.

> Sophistication was a dial we set, and you could fairly say we wrote the fraud
> so of course we catch it. So we built an operator that adapts, and we made how
> much it can see of us a setting.
>
> Blind to the review queue, it drove blocking to zero in two moves and the queue
> barely moved. We published that, and said the queue is robust because it is
> invisible.
>
> **Then we let it see the queue, and that was wrong.** Full visibility erodes
> the queue twice as fast, ninety six to eighty nine, and still does not collapse
> it. Blindness was not what protected it.
>
> What visibility changes is which lever it pulls. Blind, it chases signup
> timing. Seeing, it goes straight for rotating delivery addresses, the thing our
> own detection curve says defeats us. It finds it, pushes it to the limit, and
> the queue holds anyway.
>
> Here is why, and it is weaker than we wanted. Each of its five behaviours
> changed alone costs us under four points of recall. All five at once costs
> thirty nine, five times the sum of the parts. The destination exists. Our
> operator never assembles it, because it changes one thing at a time and any one
> thing looks like noise.
>
> That is a search failure by the attacker, not a property of our defence.
...
>
> Sealed holdout, opened once. The hardest tier: fresh device and fresh address
> for every account, signups over six weeks, some accounts behaving like real
> customers.
>
...
> stops, what it costs when it is wrong, who it depends on, and which of its own
> published claims it went back and disproved.
