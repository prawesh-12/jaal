#!/usr/bin/env bash
#
# The whole pipeline, in order. ./run.sh for the published run, ./run.sh quick
# for a smaller one.
...
if [ "$MODE" = "quick" ]; then
  echo "quick overwrites results/*.json. Restore with: git checkout results/"
fi
...
run "Linkage weights, m and u"       -m detector.link_train \
    --accounts "$ACCOUNTS" --seeds "$LINK_SEEDS" --out results/link_params.json

run "Pair scoring and ablation"      -m detector.link_eval \
...
run "Feature table, validation"      -m detector.features \
...
# A holdout opened twice is not a holdout.
if [ -f results/holdout.json ]; then
  echo
  echo "== Sealed holdout already opened, not re-running"
else
...
if [ -f results/holdout.json ] && [ -f results/features_holdout.csv ]; then
  run "Review queue and capacity"    -m detector.review
...
run "Every measured number"          -m detector.report \
